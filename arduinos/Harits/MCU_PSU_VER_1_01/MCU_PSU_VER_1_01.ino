/*
 Name:		MCU_PSU_VER_1_01.ino
 Created:	6/2/2022 12:21:45
 Author:	harit
*/
...
#define software_version F("VER.1.01")
...
//volatile boolean master_available = true;
//uint32_t auto_restart_check_interval = 60000; 
uint32_t auto_restart_threshold = 300000;
//uint32_t auto_restart_check_interval_tc = millis();
uint32_t auto_restart_threshold_tc = millis();

void auto_restart_service() {
	if (auto_restart) {
		//if (master_available) {
		//	if (millis() - auto_restart_check_interval_tc > auto_restart_check_interval) {
		//		master_available = false;
		//	}
		//}
		//else if (!master_available) {
		//if (!master_available) {
			if (millis() - auto_restart_threshold_tc > auto_restart_threshold) {
				uint32_t halt = 5000;
				byte retry = 5;
				for (byte i = 1; i <= retry; i++)
				{
					//mySerial.print('$' + (String)(token[0]) + ',' + (String)halt + '#');
					mySerial.print("$RESTART," + (String)halt + '#');
					//Serial.print("$RESTART," + (String)halt + '#');
					Serial.print('$' + (String)device_ID + ",RESTART," + (String)halt + '#');
					delay(100);
					...
					if (i >= retry) {
						...
						delay(1000);
						mySerial.begin(9600);
						auto_restart_threshold_tc = millis();
					}
				}
			}
		//}
...
void serialEvent() {
	...
	if (Serial.read() == '$') {
		...
		if (valid) {
			...
			else if ((String)token[0] == "DEVMODE" && token_count == 2) { //CHANGE DEVELOPER MODE STATE
			...
			else if ((String)token[0] == "AUTO_RESTART" && token_count == 2) { //ENABLE AUTO RESTART
				if ((String)token[1] == "ON") {
					auto_restart = true;
					//auto_restart_check_interval_tc = millis();
					auto_restart_threshold_tc = millis();
					Serial.println('$' + (String)device_ID + ",AUTO_RESTART,ON#");
					ledBlink();
				}
				else if ((String)token[1] == "OFF") {
					auto_restart = false;
					Serial.print('$' + (String)device_ID + ",AUTO_RESTART,OFF#");
					ledBlink();
				}
				else print_invalidcmd();
			}

			else if ((String)token[0] == "CHECKING" && token_count == 1) { //CHECK AVAILABILITY OF MASTER
				if (auto_restart) {
					//master_available = true;
					//auto_restart_check_interval_tc = millis();
					auto_restart_threshold_tc = millis();
					Serial.print('$' + (String)device_ID + ",CHECKING_SUCCEED#");
					ledBlink();
				}
				else if (!auto_restart) {
					Serial.print('$' + (String)device_ID + ",AUTO_RESTART_OFF#");
					ledBlink();
				}
