...
try:
    ...
    print("[X]  N4AIA04 ID: " + str(sys.argv[1]) + " " + e)
...
def connect_sensor():
    ...
    try:
        ...
        value = rs485.read_registers(0,4,3)
        if(is_connect == False):
            is_connect = True
            print("[V] N4AIA04 " + sensor_reader[0] + " CONNECTED")
...
try:
    while True:
        try:
            val = connect_sensor()
            VAL = "CEMS_N4AIA04;" + str(val[0]) + ";" + str(val[1]) + ";" + str(val[2]) + ";" + str(val[3]) + ";" + "END;"            
            update_sensor_value(str(sys.argv[1]),str(VAL))
        except Exception as e2:
            print(e2)
            is_connect = False
            print("Reconnect N4AIA04");
