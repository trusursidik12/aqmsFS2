...
try:
    ...
    print("[X]  PARTICLE COUNTER ID: " + str(sys.argv[1]) + " " + e)
...
def connect_sensor():
    ...
    try:
        ...
        regConcentration = rs485.read_registers(127,2,3)
        if(is_connect == False):
            is_connect = True
            print("[V] PARTICLE COUNTER " + sensor_reader[0] + " CONNECTED")
...
try:
    while True:
        try:
            val = connect_sensor()
            PM = "PP22_PARTICLE_COUNTER;" + str(val[0]) + ";" + str(val[1]) + ";" + "END;"            
            update_sensor_value(str(sys.argv[1]),str(PM))
        except Exception as e2:
            print(e2)
            is_connect = False
            print("Reconnect PARTICLE COUNTER");
